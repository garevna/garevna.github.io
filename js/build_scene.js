...
function show_src(i)
{
   ...
   switch(src[i].type) {
      case $items[0].type:
	      $url = $items[0].pref + src[i].ref + $items[0].suf1;
          window.open($url, '_blank')
          break;
      case $items[1].type:
          $url = $items[1].pref + src[i].ref + $items[1].suf1 + window.innerHeight + $items[1].suf2;
	      document.getElementById("scene").innerHTML = $url;
          break;
      case $items[2].type:
          $url  = $items[2].pref + src[i].ref + $items[2].suf1 + src[i].ref + $items[2].suf2;
	      $url += src[i].ref + $items[2].suf3 + src[i].ref + $items[2].suf4;
	      document.getElementById("scene").innerHTML = $url;
          break;
      case $items[3].type:
	      $url = $items[3].pref + src[i].ref + $items[3].suf1;
	      window.open($url, '_blank');
          break;
      case $items[4].type:
		  var c = src[i].ref;
...
function get_button_type(el_num)
{
	...
	{
		s = (src[el_num].type === $items[k].type)?($items[k].button_type):(s);
	}
	if (s === 'none') { alert('Не определен тип элемента меню:' + src[el_num].item_name); }
	else
	{ 
	   src[el_num].button_type = (s === undefined)?(src[el_num].button_type):(s);
	}
}
// ---------------------------------------------------------------------------------------------
function hide_panel()
...
function build_panel_structure()
{
	...
	for (var j=0; j<src.length; j++)
	{
		...
		{
			if (folders[k]._name === src[j].folder_name) { folder_num = k; break; }
		}
		if (folder_num === -1) // not found
		{
			var _folder = { _name:'', _hidden:false, _brief:false, _content:[] };
			_folder._name = src[j].folder_name;
...
// ------------------------------------------------------------------------------------
function build_scene_html(panel_title)
{
	//
	// Функция возвращает html-код страницы вместе с панелью меню
	//
	build_panel_structure();
	title = (panel_title === undefined)?(""):(panel_title);
	var $html = '<div id="container"><div class="flex-div"><div id="scene" width="98%"></div>';
	...
	for (var j=0; j < folders.length; j++) { $html += build_folder(j) }
...
function build_folder(folder_num)
{
	...
	$html += '<div id="folder' + folder_num + '_content">' + get_folder_content(folder_num);
...
function get_folder_content(folder_num)
{
	...
	for (var i=0; i < folders[folder_num]._content.length; i++)
	{
		...
		src[$src_num].h_text = (src[$src_num].h_text === undefined)?(src[$src_num].item_name):(src[$src_num].h_text);
		$html += ($brief)?('<span'):('<div');
		$html += ' class="work-panel-elem">';
		$html += '<button class="button_' + src[$src_num].button_type + '"';
		$html += ' onMouseOver="create_hlp(event,' + $src_num + ')"'; 
		$html += ' onMouseOut="remove_hlp()"';
		$html += ' onClick="show_src(' + $src_num + ')"></button>&nbsp;&nbsp;';
		$html += ($brief)?(''):(src[$src_num].item_name);
...
// ------------------------------------------------------------------------------------	
function create_hlp(event, elem_num)
{
	...
	var txt = (src[elem_num].h_text === undefined)?(src[elem_num].item_name):(src[elem_num].h_text);
...
function get_x(x)
{
	x = (x > window.innerWidth-150)?(x-150):(x);
	return x;
}
function get_y(y)
{
	y = (y > window.innerHeight-50)?(y-50):(y);
	return y;
}
